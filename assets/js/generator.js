...
$('#createButton').click(function(event) {
    ...
    for (var i = 0; i < poemArr.length; i++) {
        ...
        var randomIndex = Math.floor(Math.random()*poemArr[i].length);
        var randomArray = [poemArr[i][randomIndex]].join('');

        poemArea.append('<button id="poemButton'+ itemIndex++ +'" type="button" class="poemButton col-lg-12 btn-sm">'+randomArray+'</button>');
    }
});

  $(document).on('click', '.poemButton', function(e) {
      console.log(this);

      e.preventDefault();

      poemArea.html('');
      poemArea.append(this);
